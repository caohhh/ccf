...
DFG* DFG::Copy()
{
  ...
  for (int i = 0; i < (int) _constants.size(); i++)
  {
    ...
    tempNode->setDatatype(_constants[i]->getDatatype());
  }
  ...
  retVal->SetII(this->II); 
  return retVal;
}

void DFG::SetII(int II)
{
  this->II = II; 
}

void DFG::SetResources()
{
  ...
  for(int i=0; i < MAX_SCHEDULE_LEN; i++)
  {
    Total_Resources[i] = 0;
    Data_BUS [i]       = 0;
    Address_BUS[i]     = 0;
...
void DFG::Dot_Print_DFG_With_Schedule(string filename, int ID)
{
  ofstream dotFile;
  ostringstream graphname;
...
void DFG::Dot_Print_DFG_With_Schedule1(vector<Node*> nodes, string filename, int ID)
{ 
  ofstream dotFile;
  ostringstream graphname;
  graphname << filename << "_" << ID<<".dot";
  ...
  for (int i = 0; i < (int) _node_Set.size(); i++)
  {
    ...
    for(int j=0; j < (int) nodes.size(); j++)
    {
      if(_node_Set[i]->get_ID() == nodes[j]->get_ID())
      {
        if (_node_Set[i]->is_Mem_Operation())
        {
          dotFile << "\n" << _node_Set[i]->get_ID() << " [style=bold, color=blue ];\n";
        }
        else
        {
          ...
          if (tempDummyNode != NULL)
          {
            dotFile << "\n" << _node_Set[i]->get_ID() << " [style=bold, color=green ];\n";
          }
          else
          {
            dotFile << "\n" << _node_Set[i]->get_ID() << " [style=bold, color=red ];\n";
          }
        }
      }
      else
  ...
  for (int i = 0; i < (int) _ARC_Set.size(); i++)
  {
    switch (_ARC_Set[i]->get_Dependency())
    {
      case PredDep:
        if (_ARC_Set[i]->get_Distance() == 0)
        {
          dotFile << _ARC_Set[i]->get_From_Node()->get_ID() << " -> " << _ARC_Set[i]->get_To_Node()->get_ID() << " [style=bold, color=green ]\n";
        }
        else
        {
          dotFile << _ARC_Set[i]->get_From_Node()->get_ID() << " -> " << _ARC_Set[i]->get_To_Node()->get_ID() << " [style=dotted, color=green, label=" << _ARC_Set[i]->get_Distance() << "] \n";
...
int DFG::Init_Schedule(int id, int number_of_resources, int rec_MII)
{
  ...
  cout << "Max schedule time: " << MAX_SCHEDULE_LEN << endl;
 ...
 */
int DFG::Schedule(int id, int number_of_resources)
{
  ...
  while (true)
  {
    ...
    bool modulo_schedule = Modulo_Schedule(redo, number_of_resources);
    debugfile <<  _node_Set.size() << endl;
    if (modulo_schedule)
    {
...
int DFG::Schedule_CRIMSON(int id, int number_of_resources)
{
  ...
  while (redo && II_attempt < MAPPING_POLICY.MODULO_SCHEDULING_ATTEMPTS)
  {
    ...
    if (modulo_schedule)
    {
      schedule_len = last_time;
      debugfile << "Feasible II is " << II << "\tSchedule Len is " << schedule_len << "\n";
      DEBUG_SCHEDULING("Feasible II is %d at %d", II, II_attempt);
      for(int i=0; i < (int) _node_Set.size(); i++)
        debugfile << "Node: " << _node_Set[i]->get_ID() << "\t ASAP: " << _node_Set[i]->get_Sched_Info()->get_Feasible_ASAP() << "\tALAP: " << _node_Set[i]->get_Sched_Info()->get_Feasible_ALAP() << "\t is init: " << _node_Set[i]->get_Sched_Info()->is_CURRENT_Initiated() << "\tsched @ " << _node_Set[i]->get_Sched_Info()->get_Current()  << "\t mod: " << _node_Set[i]->get_Sched_Info()->get_Modulo_Current() << endl;  
    ...
    if (redo)
    {
      ...
      if (!Schedule_ALAP_Feasible(last_time, number_of_resources, II))
      {
        FATAL(true, "No Feasible ALAP sched found %d\n", MAX_SCHEDULE_LEN);
        return -1;
      }
      DEBUG_SCHEDULING("Multiple Constraint %d\n", II);
      II_attempt++;

      if(II_attempt >= MAPPING_POLICY.MODULO_SCHEDULING_ATTEMPTS)
        return -1; 
    }
    // no valid schedule is found
    else
  ...
  //FATAL(true, "How did we get here? attempt: %d, redo: %d", II_attempt, redo);
  return -1;
}
...
bool DFG::Modulo_Insert_Route(int &id, int number_of_resources, vector<Node*> &to_change_set, vector<Node*> &route_node)
{
  debugfile << "Left Nodes Are: ";
  for (int i = 0; i < (int) to_change_set.size(); i++)
    debugfile << to_change_set[i]->get_ID() << "\t";
  ...
  for(int i=0; i< (int) to_change_set.size(); i++)
  {
    ...
    vector<Node*> succ;
    Node* temp;
    int current_node_schedule=0; 
    vector<int> distance; 
    bool is_constrained = startNode->get_dist_physical_successors_constrain_by_this_modulo(II,distance,succ);
    ...
    if(is_constrained)
    {
      for(int j=0; j < (int) distance.size(); j++)
      {
        ...
        // If distance is less than 0 (negative) then the routing nodes cannot be added
        // because the successor precedes the current node in modulo schedule. 
        // We have to increase the II then or try a new schedule.
        if(distance[j] < 0)
        {
          // This II is not feasible. return failure
          _FATAL("distance in negative for node:%d\t succ:%d\n", temp->get_ID(), succ[j]->get_ID());
          return false;
        }
        ...
        if(k>0)
        {
          ...
          {
            //Get current node's asap and alap
            if(ii==0)
            ...
            dummy_node->setDatatype(temp->getDatatype()); 
            routenode.push_back(dummy_node);

            // insert the routing node
            insert_Node(routenode[ii]);   
            if ((int)routenode[ii]->get_Number_of_Pred()>0) Add_Arc(routenode[ii]->get_Pred_Arc(0));
            insert_Node_in_between_input(temp, succ[j], routenode[ii]);
            // Update the timing of the nodes.
  ...
  // Sometimes there are two arcs added to the routing nodes.
...
int DFG::set_height_based_priority()
{
  DEBUG_SCHEDULING("Priority Started");
  //set of scheduled operations
  vector<Node*> prioritized;
  //set of operations selected to be scheduled
  vector<Node*> to_be_prioritized;
  //set of unscheduled operations
  vector<Node*> rest = _node_Set;
  //initially nodes without successor at current iteration are to be scheduled
  to_be_prioritized = get_set_of_end_nodes();

  int priority = 0;
  int highest_priority = 0; 
  //schedule those nodes at cycle given as parameter
  for (int i = 0; i < (int) to_be_prioritized.size(); i++)
  {
    to_be_prioritized[i]->get_Priority_Info()->set_Height(0);
    // DEBUG_ALAP_SCHEDULING("Node %d is scheduled at time %d", rest[i]->get_ID(), latest_time);
  }
  //update set of scheduled operations
  prioritized = Union(prioritized, to_be_prioritized);
  //update set of unscheduled operations
  rest = Subtract(rest, prioritized);
  //Subtract1(rest, scheduled); 
  //while there is an unscheduled operation
  while ((int) rest.size() > 0)
  {
    ...
    {
      //is it ready to be schedule? if yes, at which cycle?
      if (rest[i]->is_ready_to_prioritize(priority))
      {
        //schedule it at scheule_time cycle
        rest[i]->get_Priority_Info()->set_Height(priority);
        //DEBUG_ALAP_SCHEDULING("Node %d is scheduled at time %d", rest[i]->get_ID(), schedule_time);
        //update set of scheduled operations
        prioritized.push_back(rest[i]);
      }
    }
    rest = Subtract(rest, prioritized);
    //Subtract1(rest, scheduled);
...
bool DFG::is_Executable(int height, int CGRASize)
{

  vector<int> node_count(height, 0); 
  vector<Node*> rest = _node_Set; 


  // populate the height vector (node_count at each height level)
  for(int i=0; i < (int) rest.size(); i++)
    if(rest[i]->get_Priority_Info()->is_Height_Initiated())
      node_count[rest[i]->get_Priority_Info()->get_Height()]++;

  // add routing edges(routing resoures required) for the height. 
  // TODO: if we add routing resources in architecture make resources
  //        into a seperate array and then code accordingly.

  for(int i=0; i < (int)rest.size(); i++)
  {
    vector<int> distance; 
    vector<Node*> nodes;
    if(rest[i]->get_dist_height_constraint(distance, nodes))
    {
      int distance1 = rest[i]->get_Priority_Info()->get_Height(); 
      ...
      for(int j=0; j < (int) nodes.size() ; j++)
      {
        ...
        {
          for(int l=1;l<=k;l++)
            node_count[distance1-l]++;  
        }
      }
    }    
  }

  // finally, check if the rach height has enough resources.

  for(int i=0; i < height; i++)
  {
    if(node_count[i] > CGRASize)
      return false;
  }
...
bool DFG::connected_Nodes2(Node* node1, Node* node2, int II)
{
...
int DFG::get_Node_Index(Node * node1)
{
...
void DFG::GenerateAdjacencyForPath(int II)
{

  int nodeSize = (int) _node_Set.size();
  adj = new list<int>[nodeSize];
  vector<Node*> nodes = this->getSetOfVertices();  
  ...
  for(int i=0; i < nodeSize; i++)
  {
    for(int j=0; j < nodeSize; j++)
    {
      ...
      if(connected_Nodes2(nodes[i], nodes[j], II))
      {
...
int DFG::GetPath(int s, int d, int V) 
{ 
  ...
  GetPathUtil(s, d, visited, path, path_index); 

  int counts = count(paths.begin(), paths.end(), s);
  ...
  all_paths = new list<int>[V];
  int j=-1;
  for(int i = 0; i < (int) paths.size(); i++)
  {
    if(paths[i] == s)
      j++;
    all_paths[j].push_back(paths[i]); 
  }
...
// index in path[] 
void DFG::GetPathUtil(int u, int d, bool visited[], int path[], int &path_index) 
{ 
  ...
  { 
    ...
    for (i = adj[u].begin(); i != adj[u].end(); ++i) 
      if (!visited[*i]) 
        GetPathUtil(*i, d, visited, path, path_index); 
  } 
...
void DFG::Prune_Routing(DFG* original, int II)
{

  //DFG* retVal = Copy_With_Schedule(); 
  GenerateAdjacencyForPath(II);  
  vector<Node*> original_node_set = original->getSetOfVertices(); 
  vector<Node*> temp_node_set = this->getSetOfVertices();
  int temp_nodeSize = (int) temp_node_set.size(); 


  for(int i=0; i < (int) original_node_set.size(); i++)
  {
    Node* temp_node = get_Node(original_node_set[i]->get_ID()); 
    int temp_node_modulo_time = temp_node->get_Sched_Info()->get_Modulo_Current(); 
    vector<Node*> Outgoing = original_node_set[i]->Get_Next_Nodes();

    // a. For each node and outgoing pair get the Modulo schedule time.
    // b. Get if the pair is scheduled more than a cycle apart, and if the cycles are <  II then continue
    //    Otherwise i.e., if it = to II then prune the routing nodes if any. 

    for(int j=0; j < (int) Outgoing.size(); j++)
    {
      cout << "Node: "<< original_node_set[i]->get_ID() << "\tOut: " << Outgoing[j]->get_ID() << endl;
      Node* temp_out_node = get_Node(Outgoing[j]->get_ID());
      int temp_out_modulo_time = temp_out_node->get_Sched_Info()->get_Modulo_Current();

      ARC* arc1 = original->get_Arc(temp_node, temp_out_node);

      bool recurrent = false; 

      if(arc1->get_Distance() > 0)
        recurrent = true;  
      // if the node-outgoing pair is schedule more than 1 cycle apart in the current graph
      if( recurrent && abs(temp_node_modulo_time - temp_out_modulo_time) == (II-1))
      {
        cout << "inside if abs for nodes: " << temp_node->get_ID() << " & " << temp_out_node->get_ID() << endl;
        int num_paths = GetPath(get_Node_Index(temp_node), get_Node_Index(temp_out_node), temp_nodeSize); 

        for(int k=0; k < num_paths; k++)
        {
          cout << k << "---> " ;
          for(list<int>::iterator iter = all_paths[k].begin(); iter != all_paths[k].end(); ++iter)
            cout << *iter << " "; 

          cout << endl;
        }

        int routing_ops[num_paths]; // record the num of routing nodes added per path already.
        ...
        for(int k = 0 ; k < num_paths; k++)
        {
          for(list<int>::iterator ii = all_paths[k].begin(); ii != all_paths[k].end(); ++ii)
          {
            if(*ii != get_Node_Index(temp_node) || *ii != get_Node_Index(temp_out_node))
            {
              cout << "*ii from routing ops " << *ii << endl;
              if(get_Node_from_Set(*ii)->get_Instruction() == route_op)
                routing_ops[k]++; 
            }
          }
        }
        ...
        for(int k=0; k < num_paths; k++)
          cout << "routingops: " << routing_ops[k] << endl;

        bool prune = false;
        vector<int> prune_paths; 
        if(!recurrent)
        {
          cout << "inside not recurrent" << endl;
          for(int k = 0 ; k < num_paths; k++)
          {
            if(routing_ops[k] >= II-1)
            {
              cout << "inside prune path -- non recurrent" << endl;
              prune_paths.push_back(k); 
              prune = true; 
            }
          }
        } 
        ...
        {
          cout << "inside recurrent" << endl;
          for(int k = 0 ; k < num_paths; k++)
          {
            if(routing_ops[k] > 0)
            {
              cout << "inside prune path else recurrent" << endl;
              prune_paths.push_back(k);
              prune = true; 
            }
          }
        ...
        if(prune)
        {
          for(int k = 0; k < (int) prune_paths.size(); k++)
          {
            //list<int>::iterator ii =  all_paths[k].begin();
            list<int>::iterator jj = all_paths[k].end(); 
            advance(jj, -2);

            cout << "from " << get_Node_from_Set(*jj)->get_ID() << "--->" << temp_out_node->get_ID() << endl;
            // get the arc characteristics of the last node of the path and temp_out_node
            ARC* arc = get_Arc(get_Node_from_Set(*jj), temp_out_node);

            cout << "arc dist: " << arc->get_Distance() << "\tarc dep: " << arc->get_Dependency() << "\t arc op: " << arc->get_OperandOrder() << endl; 
            // make the arc from the temp_node to temp_out_node
            make_Arc(temp_node, temp_out_node, ++ARC_Max_Index, arc->get_Distance(), arc->get_Dependency(), arc->get_OperandOrder());

            //Remove arc between the last node and the last before nodes
            Remove_Arc(get_Node_from_Set(*jj), temp_out_node); 

            vector<Node*> path_nodes; 
            for(list<int>::iterator iter = all_paths[k].begin(); iter != all_paths[k].end(); ++iter)
              path_nodes.push_back(get_Node_from_Set(*iter)); 

            for(int jj=0; jj < (int) path_nodes.size(); jj++)
            {
              // the first and the last nodes are the source and destination nodes
              if(path_nodes[jj]->get_ID() == temp_node->get_ID() || path_nodes[jj]->get_ID() == temp_out_node->get_ID())
                continue; 

              if(path_nodes[jj]->get_Instruction() == route_op)
                Remove_Arc(path_nodes[jj-1], path_nodes[jj]);
            }
...
bool DFG::Modulo_Schedule(bool &multiple_constraint, int number_of_resources)
{
  int tempII = II;
  multiple_constraint = false;  //initially no node is constrained by multiple predecessor

  // Set all the resources required for scheduling.
  SetResources(); 
...
bool DFG::Modulo_Schedule_Routing(int number_of_resources, vector<Node*> to_schedule)
{
  int tempII = II;
  ...
  debugfile << "rec_MII="  << recMII << "\tresMII=" << resMII << "\tII:" << tempII << endl; 
  ...
  while((int) to_be_scheduled.size() > 0)
  {
    ...
    debugfile << "currently scheduling route node: " << ready[0]->get_ID() <<endl;
    ...
    for (int i = schedule_time; i >= end; i--)
    {
      ...
      else
      {
        ...
        cout << "Not enough resources available at " << schedule_time << " PE used " << Total_Resources[modulo_schedule_time] << endl;
        for(int jj=0; jj<II; jj++)
          cout << "Resources @ " << jj << "\t" << Total_Resources[jj] << endl; 
...
bool DFG::Modulo_Schedule_2(bool &multiple_constraint, int number_of_resources)
{
  int tempII = II;
  ...
  while ((int) to_be_scheduled.size() > 0)
  {
    ...
    // The ASAP of the start_time of the current node, hence, should at least be, 
    // the greatest of all the predecessors mapped + 1.
    int start_time = MinTime(ready_sorted[0]);
    if(start_time == INT_MAX)
      start_time = ready_sorted[0]->get_Sched_Info()->get_Feasible_ASAP();
    ...
    if((int) LCD.size() > 0)
    {
      ...
      { 
	    ...
	    return false;
          ...
          while(diff1 <= 0 && try_time < end_time)
          {
            int new_try_time = try_time++;
    ...
    while(!node_scheduled)
    {
      ...
      if (ready_sorted[0]->is_Load_Address_Generator())
      {
        ...
        if (multiple_constraint_schedule)
        {
          cout << "Load Address generator multiple constraint\n";
          multiple_constraint = true;
          return false;
        }
        //if succeed, update list of nodes
      ...
      else if (ready_sorted[0]->is_Load_Data_Bus_Read())
      {
        ...
        /*if (multiple_constraint_schedule)
          {
          cout << "Load Data multiple constraint\n";
          multiple_constraint = true;
          return false;
          }*/
        //if succeed, update set of nodes
...
int DFG::MinTime(Node* current)
{
  //temp storage to travers DFG
  vector<Node*> visited;
  //find successors of scr in this iteration
  vector<Node*> predecessors = current->Get_Prev_Nodes_Same_Iteration();
  // return min time
  int mintime = INT_MAX; 
  ...
  for (int i = 0; i < (int) predecessors.size(); i++)
  {
    ...
    if (traverse_constrain_path_recursive(current, predecessors[i], visited))
    {
      //calculate the mintime (start time) based on all predecessors scheduled
      mintime = (predecessors[i]->get_Sched_Info()->get_Current() < mintime) ? predecessors[i]->get_Sched_Info()->get_Current() : mintime; 
    } 
    visited.clear();
  }
  if(mintime == INT_MAX)
...
bool DFG::constrains_multiple(int II, Node* current, int schedule_time, Node* &first_node_in_constrain_path, vector<Node*> &scheduled_nodes)
{
  ...
  for (int i=0; i <(int)scheduled_nodes.size(); i++)
  {
    ...
    {
      cout << "constrains multiple() src node: " << current->get_ID() << "\ttarget: " << scheduled_nodes[i]->get_ID() << endl;
      counter++;
    }
  }
...
bool DFG::constrains(int II, Node* src, int src_schedule_cycle, Node* target, int target_schedule_cycle, int target_modulo_cycle, Node* &first_node_in_conflict_path)
{
  ...
  for (int i = 0; i < (int) successors.size(); i++)
  {
    ...
    if (src_schedule_cycle < updated_successor_cycle - src->get_Latency())
    {
      ...
      cout << "edge distance: " << edge->get_Distance() << endl;
      cout << "src node: " << src->get_ID() << "\ttar: " << target->get_ID() << endl;
      cout << "src scheduled:" << src_schedule_cycle << "\ttar scheduled: " << updated_successor_cycle - src->get_Latency() << endl;
      //can reach target from next?
      if (traverse_constrain_path_recursive(successors[i], target, visited))
      {
        first_node_in_conflict_path = successors[i];
...
DFG* DFG::Copy_With_Schedule_And_Mapping()
{
  ...
  retVal->MAX_SCHEDULE_LEN = MAX_SCHEDULE_LEN;
  return retVal;
}
    ...
    if(_node_Set[i]->getLoadConstant())
    {
      ...
      for(it = _Const_ARC_Set.begin(); it != _Const_ARC_Set.end(); it++)
        ...
        else if(std::get<0>(*it) == _node_Set[i]->get_ID())
        {
          ...
          store_address->set_Store_Address_Generator(store_data);
          store_data->set_Store_Data_Bus_Write(store_address);
          store_address->setDatatype(dt);
          store_data->setDatatype(dt);
          //Insert All Nodes
          insert_Node(store_address);
          insert_Node(store_data);
...
void DFG::Copy_Resource_Info(int* totalResources, int* addBus, int* dataBus, int maxSchedLen)
{
  ...
  debugfile << "inside copy max sched " << maxSchedLen << endl; 
  for(int i=0; i < maxSchedLen; i++)
  {
    debugfile << "i: " << i << endl;
    Total_Resources_info[i] = totalResources[i];
    debugfile << "passed resources" << endl;
    Data_BUS_info[i]       = dataBus[i];
    debugfile << "passed databus" << endl;
    Address_BUS_info[i]     = addBus[i];
    debugfile << "passed address" << endl;
  }
}
