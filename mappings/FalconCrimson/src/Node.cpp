...
Node::Node(Instruction_Operation ins, int laten, int id, string nodeName = " ")
{
  ...
  height_priority = new PriorityHeight(); 
...
//return the set of incoming edges with distance
vector<Node*> Node::Get_Prev_Nodes_Next_Iteration()
{
  vector<Node*> retVal;
  int SizeOfInComing = inCommingArcs.size();
  for (int i = 0; i < SizeOfInComing; i++)
  {
    if (inCommingArcs[i]->get_Distance() > 0)
      retVal.push_back(inCommingArcs[i]->get_From_Node());
  }
...
bool Node::is_Physically_Connected2_To(Node* nextNode)
{
  if (nextNode == this && self_loop)
    return true;
  vector<Node*> nextNodes = Get_Next_Nodes();
...
PriorityHeight* Node::get_Priority_Info()
{
...
tuple<bool,int> Node::debug_is_ready_to_schedule_ASAP(int &time)
{
  time = 0;
  int current_time;
  vector<Node*> prev_nodes = Get_Predecessors_Same_Iteration_Exclude_Address_Dependency();
  cout << "prev_size: " << (int) prev_nodes.size() << endl;
  for(int j=0; j < (int) prev_nodes.size(); j++)
        cout << "prev node: " << prev_nodes[j]->get_ID() << endl;
...
bool Node::is_ready_to_prioritize(int &time)
{
  time = MAXINT;
  int current_time;
  vector<Node*> next_nodes = Get_Next_Nodes_Same_Iteration(); //Get_Successors_Same_Iteration_Exclude_Address_Dependency();
  for (int i = 0; i < (int) next_nodes.size(); i++)
  {
    if (!next_nodes[i]->get_Priority_Info()->is_Height_Initiated())
      return false;
    current_time = next_nodes[i]->get_Priority_Info()->get_Height() + 1;
    if (current_time < time)
      time = current_time;
  }
  return true;
...
vector<Node*> Node::get_physical_successors_constrain_by_this_modulo2(int II)
{
  vector<Node*> retVal;
  int SizeOfIncomming = outGoingArcs.size();
  ...
  for (int i = 0; i < SizeOfIncomming; i++)
  {
    if (outGoingArcs[i]->get_Dependency()!= TrueDep && outGoingArcs[i]->get_Dependency()!= PredDep && outGoingArcs[i]->get_Dependency()!= LoadDep)
      continue;
...
vector<Node*> Node::get_physical_predecessors_constrain_this_modulo2(int II)
{
  vector<Node*> retVal;
  int SizeOfIncomming = inCommingArcs.size();
  for (int i = 0; i < SizeOfIncomming; i++)
  {
    if (inCommingArcs[i]->get_Dependency()!= TrueDep && inCommingArcs[i]->get_Dependency()!= PredDep && inCommingArcs[i]->get_Dependency()!= LoadDep)
      continue;
...
bool Node::get_dist_height_constraint(vector<int> &distance, vector<Node*> &nodes)
{
  vector<Node*> Outgoing = Get_Next_Nodes(); 
  bool retVal = false;
  ...
  {
    int edge_height = Outgoing[i]->get_Priority_Info()->get_Height(); 
    int distance1 = this->get_Priority_Info()->get_Height() - edge_height;

    if(distance1 > 1)
    {  
      distance.push_back(distance1);
      nodes.push_back(Outgoing[i]);
      retVal = true;  
    }
  }
...
bool Node::get_dist_physical_successors_constrain_by_this_modulo(int II, vector<int> &distance, vector<Node*> &nodes)
{
  ...
  for (int i = 0; i < SizeOfIncomming; i++)
  {
    ...
    if((distance1 > this->get_Latency()) || (distance2 > this->get_Latency()) || (edge_distance > 0))
    {
      ...
      {
        // This is a double pointer search to insert the value.
        unsigned int j=0;
        int k=0,l=distance.size()-1; 
        while(k!=l)
        {
          if(distance[k] >= temp)
          {
            //distance.insert(distance.begin()+k+1,1,temp);
            //nodes.insert(nodes.begin()+k+1,1,outGoingArcs[i]->get_To_Node());
            j = k;
            break;
          }
          else
          ...
          if(distance[l] <= temp)
          {
            //distance.insert(distance.begin()+l+1,1,temp);
            //nodes.insert(nodes.begin()+l+1,1,outGoingArcs[i]->get_To_Node()); 
            j = l;
            break;
          } 
          else
...
bool Node::get_dist_physical_successors_constrain_by_this_Feasible_ASAP(int II, vector<int> &distance, vector<Node*> &nodes)
{
  int SizeOfIncomming = outGoingArcs.size();
  bool retVal = false;
  ...
  for (int i = 0; i < SizeOfIncomming; i++)
  {
    ...
    if(edge_distance > 0)
    {
      distance1 = outGoingArcs[i]->get_To_Node()->get_Sched_Info()->get_Feasible_ASAP() - this->get_Sched_Info()->get_Feasible_ASAP();
      distance1 += II*edge_distance;
    }
    int distance2 = outGoingArcs[i]->get_To_Node()->get_Sched_Info()->get_Feasible_ASAP() - this->get_Sched_Info()->get_Feasible_ASAP();
    if((distance1 > this->get_Latency()) || (distance2 > this->get_Latency()) || (edge_distance > 0))
    {
      int temp = (distance1 > distance2) ? distance1 : distance2;
      //sort the pairs in terms of scheduling time or distance2 (for path sharing purposes)
...
void Node::SetMappableIndices(vector<MappingPair*> positions)
{
  // clear out the previous indices.
  indices.clear(); 
...
void Node::SetTempMappableIndices(vector<MappingPair*> positions)
{
  // clear out the previuous indiuces
  temp_best_indices.clear(); 

  for(int i=0;i < (int) positions.size(); i++)
    temp_best_indices.push_back(positions[i]);
}

    //get the indices while mapping
vector<MappingPair*> Node::GetMappedIndices()
{
  return indices;
}

vector<MappingPair*> Node::GetTempMappedIndices()
{
  return temp_best_indices;
}
    // 
...
MappingPair* Node::GetTempCurrentPosition()
{
  return temp_pos;
}
