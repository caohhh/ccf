...
Parser::ParseDFG(DFG* myDFG)
{
  ...
  if (fin_node.is_open()) {
    while (fin_node.getline(&line[0], 255, '\n')) {
      ...
      strout >> nodeID >> node_type >> nodeName >> temp >> datatype;
      
      if (nodeName == "ft_tab" || nodeName.find("dmy_CGRAst") != string::npos || nodeName.find("amb_temp") != string::npos)
        nodeName = "ConstInt0"; 
      ...
      if (!node_exists) {
        ...
        pNode->setDatatype((Datatype)datatype);
...
string 
Parser::getNodeFileName()
{
  return this->nodeFile;
}

string 
Parser::getEdgeFileName()
{
