...
class Node
{
  private:
    ...
    Datatype ins_datatype; // datatype update
    int latency;
    string name;
    ...
    PriorityHeight* height_priority;
  ...
  public:
    ...
    //return the set of incoming nodes recurrent distance
    vector<Node*> Get_Prev_Nodes_Next_Iteration(); 
    ...
    bool is_Physically_Connected2_To(Node* nextNode);

    //**************************Scheduling********************************//
    Scheduling_Info* get_Sched_Info();
    PriorityHeight* get_Priority_Info();

    bool is_ready_to_schedule_ASAP(int &time);
    tuple<bool,int> debug_is_ready_to_schedule_ASAP(int &time) ;
    bool is_ready_to_schedule_ALAP(int &time);
    bool is_ready_to_prioritize(int &time);
    ...
    //set the indices while mapping
    void SetMappableIndices(vector<MappingPair*> postions);

  
    void SetTempMappableIndices(vector<MappingPair*> postions);

    //get the indices while mapping
    vector<MappingPair*> GetMappedIndices(); 

  
    vector<MappingPair*> GetTempMappedIndices();

    // set the current random position when mapping
    void SetCurrentPosition(MappingPair* pos);

    void SetTempCurrentPosition(MappingPair* pos);

    //get the current random position when mapping
    MappingPair* GetCurrentPosition();  

    MappingPair* GetTempCurrentPosition();

    vector<Node*> get_physical_successors_constrain_by_this_modulo(int II);
    vector<Node*> get_physical_successors_constrain_by_this_modulo2(int II);
    vector<Node*> get_physical_predecessors_constrain_this_modulo(int II);
    vector<Node*> get_physical_predecessors_constrain_this_modulo2(int II);
    ...
    bool get_dist_physical_successors_constrain_by_this_Feasible_ASAP(int II, vector<int> &distance, vector<Node*> &nodes);
    int get_dist(Node* node, int II);
    bool get_dist_height_constraint(vector<int> &distance, vector<Node*> &nodes);
