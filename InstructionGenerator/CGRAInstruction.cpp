...
{
  unsigned long ins_enc = InsWord & 0xffffffff; 
...
unsigned long CGRA_Instruction::getDatatype_DECODE()
{
  unsigned long InsWord = 0;
  InsWord |=DType;
  InsWord <<= SHIFT_DATATYPE;
  InsWord &= INS_DATATYPE;
  return InsWord;
}
...
{
  unsigned long InsWord =0;
  InsWord |= Ins->getDatatype_DECODE(); 
...
Datatype Pred_Instruction::getPredDatatype()
{
  return DType;
}
...
int Pred_Instruction::getPredicator()
{
  return Predicator;
}
...
{
  unsigned long ins_pred = PredInsWord & 0xffffffff; 
  ...
  Predicator= (PredInsWord & INS_PREDICT )>>SHIFT_PREDICT;
...
unsigned long Pred_Instruction::getPredDatatype_DECODE()
{
  unsigned long PredInsWord = 0; 
  PredInsWord |=DType;
  PredInsWord <<= SHIFT_PDATATYPE;
  PredInsWord &= INS_PDATATYPE;
  return PredInsWord;
}
...
{
  unsigned long PredInsWord =0;
  PredInsWord |= Ins->getPredDatatype_DECODE();  
